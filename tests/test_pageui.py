...
Unit tests for the pageui module.
"""
...
class PageTestCase(ut.TestCase):
    ...
    def test_set_frame(self):
        ...
        """
        exp = 'heavy'
        page = pageui.Page('spam', frame=exp)
        act = page.frame.kind
    ...
    def test_set_title(self):
        ...
        """
        exp = 'eggs'
        page = pageui.Page('spam', title=exp)
        act = page.title
        self.assertEqual(exp, act)
    ...
    def test_set_padding(self):
        ...
        """
        exp = 2
        page = pageui.Page('spam', padding=exp)
        act = page.padding
        self.assertEqual(exp, act)

    # Test Page.back()
    @patch('blackjack.pageui.Terminal.width', new_callable=PropertyMock)
    @patch('blackjack.pageui.Terminal.height', new_callable=PropertyMock)
    @patch('blackjack.pageui.print')
    def test_back(
        self,
        mock_print,
        mock_height,
...
class PageUITestCase(ut.TestCase):
    @patch('blackjack.pageui.Page', return_value=Mock())
    def test_init(self, mock_page):
        """When called, PageUI() should return an initialized PageUI
        object.
        """
        # Expected values.
        exp_class = pageui.PageUI
        exp_attrs = {
            'ctlr': mock_page(),
        }

        # Run test and gather actuals.
        act_obj = pageui.PageUI()
        act_attrs = act_obj.__dict__
